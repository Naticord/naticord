...
from PyQt6.QtWidgets import (
    ...
    QListWidgetItem, QTabWidget, QFontDialog, QInputDialog
)
...
class Naticord(QWidget):
    ...
    def create_settings_menu(self):
        ...
        self.top_bar_layout.addWidget(self.settings_button, alignment=Qt.AlignmentFlag.AlignRight)
            ...
            for server in servers_data:
                if server.get("name") == server_name:
                    ...
                    if response.status_code == 200:
                        ...
                        back_item.setData(Qt.ItemDataRole.UserRole, "back")
                        self.servers_list.addItem(back_item)
                        for channel in channels_data:
                            channel_name = channel.get("name")
                            self.servers_list.addItem(channel_name)
                            self.servers_list.item(self.servers_list.count() - 1).setData(Qt.ItemDataRole.UserRole, channel.get("id"))
    ...
    def display_messages(self, messages):
        ...
        for message in reversed(messages):
            ...
            message_item.setData(Qt.ItemDataRole.UserRole, QVariant(menu))
    ...
    def delete_message(self, message_id):
        ...
        confirm = QMessageBox.question(self, "Delete Message", "Are you sure you want to delete this message?", QMessageBox.StandardButton.Yes | QMessageBox.StandardButton.No)
    ...
    def search_friends(self, text):
        items = self.friends_list.findItems("*", Qt.MatchFlag.MatchWildcard | Qt.MatchFlag.MatchWrap)
        for item in items:
    ...
    def search_servers(self, text):
        items = self.servers_list.findItems("*", Qt.MatchFlag.MatchWildcard | Qt.MatchFlag.MatchWrap)
        for item in items:
            if text.lower() not in item.text().lower():
                item.setHidden(True)
            else:
